...
class YearAlbumsCollapse extends Component {
    ...
    render() {
        ...
        for (let index = 0; index < 10; index++) {
            ...
            albumsYearBox.push(box);

        }

        const div = <div className="box" key={this.state.year.key}>
            <div> <h1>{this.state.year.key} Top Albums <Button outline color="secondary" size="sm" onClick={this.toggle}>{this.state.collapse ? 'Close' : 'Open'}</Button></h1>  </div>
            <Collapse isOpen={this.state.collapse}>
                <div className="reasons"> {albumsYearBox} </div>
            </Collapse>
        </div>
...
export default YearAlbumsCollapse;
